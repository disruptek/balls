...
proc numberLinesCT(s: string; first = 1): NimNode =
  ...
  for n, line in pairs(splitLines(s, keepEol = true)):
    var ln = infix(newLit lineNumStyle, "&", align($(n + first), LNcols).newLit)
    ln = infix(ln, "&", "  ".newLit)
    ln = infix(ln, "&", infix(newLit sourceStyle, "&", line.newLit))
    result.add ln
...
proc success(t: var Test): NimNode =
  ...
  result.add t.output(infix(newLit successStyle, "&", newLit(t.name)))
...
proc failure(t: var Test; n: NimNode = nil): NimNode {.used.} =
  ...
  result = newStmtList()
  result.add t.incResults
  result.add t.output(infix(newLit failureStyle, "&", newLit(t.name)))
...
proc dotMsg(n: NimNode): NimNode =
  ...
  result = infix(newLit commentStyle, "&", newDotExpr(n, ident"msg"))
...
proc badassert(t: var Test; n: NimNode = nil): NimNode =
  ...
  if n.isNil:
    ...
    result.add t.output(infix(newLit failureStyle.string, "&", nestList(ident"&", text)))
  result.add t.renderSource
  result.add t.setExitCode
  ...
  result =
    nnkStmtList.newTreeFrom n:
      ...
      t.output:
        nestList bindSym"&":
          nnkStmtList.newTreeFrom n:
            infix(newLit skippedStyle, "&", t.name.newLit)
            newLit": "
            n.dotMsg
...
proc exception(t: var Test; n: NimNode): NimNode =
  ...
  result = newStmtList()
  result.add t.incResults
  result.add t.output(infix(newLit exceptionStyle, "&", nestList(ident"&", text)))
  result.add t.renderSource
  result.add t.renderTrace(n)
  result.add t.setExitCode
...
proc reportResults(): NimNode =
  ...
  add legend:
    comment infix(newLit resultsStyle, "&", infix(newLit($totalTests()), "&", newLit" tests  "))
...
proc postTest(test: Test): NimNode =
  ...
  when ballsAuditTimeSpace:
    ...
    add text:        # the test number with extra styling
      infix(newLit testNumStyle, "&", pad(newLit $test.number, 5))
...
proc compilerr(t: var Test): NimNode {.used.} =
  ...
  result = newStmtList()
  result.add t.incResults
  result.add t.output(infix(newLit oopsStyle, "&", newLit(t.name & ": compile failed")))
  result.add t.renderSource
  result.add t.setExitCode
